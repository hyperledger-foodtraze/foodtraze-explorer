			...
			dbStatusMetrics.getPeerList(
				...
				(data: any) => {
					...
					for (const val of data) {
						if (val.peer_type === 'PEER') {
							const obj = val;

							if (val.mspid === 'Org1MSP') {
								obj.alias_org = 'Producer'
								obj.displayIndex = 1
							} else if (val.mspid === 'Org2MSP') {
								obj.alias_org = 'Processing Plant'
								obj.displayIndex = 2
							} else if (val.mspid === 'Org4MSP') {
								obj.alias_org = 'Distribution Center'
								obj.displayIndex = 4
							} else if (val.mspid === 'Org5MSP') {
								obj.alias_org = 'Retailer'
								obj.displayIndex = 5
							}
							result.push(obj);
						}
					}
					const finalResult = sortDataBy(result)
					res.send({ status: 200, peers: finalResult });
				}
			);
		} else {
			return requtil.invalidRequest(req, res);
